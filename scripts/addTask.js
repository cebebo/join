...
function openAddTaskInBoard(use) {
    ...
    if (use == "edit") { showAddTaskButtons() };
    if (use == "add") { showEditTaskButtons() };
}

function showAddTaskButtons() {
...
async function createTask(location) {
    if (!editMode) {
        ...
        while (subtasks.length > 0) { subtasks.pop(); }
...
async function saveChanges(location, i) {
    ...
    if (location == 'IB') { closeAddTaskInBoard(); }
    initBoard();
    window.location.href = "board.html?msg=Changes saved";
    editMode = false;
}
